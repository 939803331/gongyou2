...
public class GenerateVo {
	...
	public void generator() {
		...
		String className = StringUtils.underline2Camel(table.getTableName(), false) + "Vo";
		CompilationUnit unit = vm.newCompilationUnit(generatorProperties.getPath());
		unit.setNamespace(generatorProperties.getPackagesVo());
		PackageClass clazz = unit.newClass(className);
		clazz.setAccess(Access.PUBLIC);
		clazz.setComment(Comment.DOCUMENTATION, GenerateUtils.documentation(className, generatorProperties.getPackagesVo(), table.getTableComment() + "vo"));
		...
		clazz.addAnnotation("ApiModel").addAnnotationAttribute("value", vm.newString(table.getTableComment() + "vo"));
		...
		for (Columns column : columns) {
			...
			field.addAnnotation("ApiModelProperty").addDefaultValueAttribute(vm.newString(column.getColumnComment()));
		}

		ClassField pageNo = clazz.newField(vm.newType(Integer.class), "pageNo");
		pageNo.setAccess(Access.PRIVATE);
		pageNo.addAnnotation("ApiModelProperty").addDefaultValueAttribute(vm.newString("页数"));
		pageNo.setExpression(vm.newInt(1));

		ClassField pageSize = clazz.newField(vm.newType(Integer.class), "pageSize");
		pageSize.setAccess(Access.PRIVATE);
		pageSize.addAnnotation("ApiModelProperty").addDefaultValueAttribute(vm.newString("每页条数"));
		pageSize.setExpression(vm.newInt(20));
